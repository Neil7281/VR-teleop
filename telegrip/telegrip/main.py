...
class APIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_restart_request(self):
        ...
        try:
            if hasattr(self.server, 'api_handler') and self.server.api_handler:
                logger.info("Restarting RoboInn teleoperation system...")
                self.server.api_handler.restart()
                self.send_response(200)
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({"success": True, "message": "RoboInn system restarted"}).encode('utf-8'))
            else:
                self.send_error(500, "System not available")
...
class TelegripSystem:
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info("RoboInn: all system components started successfully")
    ...
    async def stop(self):
        ...
        logger.info("RoboInn shutdown complete")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="RoboInn — VR Teleoperation System for the SO100 Robot")
    ...
    parser.add_argument("--force-sim", action="store_true", help="Keep PyBullet sim/GUI on for reBot (off by default for smooth control)")
    ...
    parser.add_argument("--calibrate", choices=["left", "right", "both"],
                        metavar="ARM", nargs="?", const="both",
                        help="Run arm calibration before starting (left/right/both)")
...
def create_config_from_args(args) -> TelegripConfig:
    ...
    # reBot teleop uses the official Pinocchio IK (not PyBullet), so the PyBullet
    # sim/GUI is pure overhead that stalls the asyncio event loop and makes motor
    # tracking jerky. Default it OFF for reBot unless the user explicitly asks
    # for it with --force-sim. SO100 (which uses PyBullet IK) is unaffected.
    if config.is_rebot and not getattr(args, "force_sim", False):
        if config.enable_pybullet:
            logger.info("reBot: disabling PyBullet sim/GUI for smooth control "
                        "(use --force-sim to keep it).")
...
async def main():
    ...
    if getattr(args, 'calibrate', None):
        from .driver.calibrator import SO100Calibrator
        arms = (["left", "right"] if args.calibrate == "both"
                else [args.calibrate])
        for arm in arms:
            if not config.arm_enabled.get(arm, True):
                print(f"⚠️  {arm.upper()} arm is disabled — skipping calibration.")
                continue
            cal = SO100Calibrator(
                port=config.follower_ports[arm],
                arm_id=arm,
                save_path=config.calibration_paths.get(arm),
            )
            cal.run()
        print("Calibration done. Starting teleoperation...\n")
    ...
    # Always show a clean startup message with the UI URL so headless/info runs
    # don't look like they're stuck after PyBullet initializes.
    print("🤖 RoboInn starting...")
    print("📱 Open the UI in your browser at:")
    print(f"   https://{host_display}:{config.https_port}")
    print("📱 Then open the same address in your VR headset browser")
    if log_level > logging.INFO:
