...
from ..config import (
    NUM_JOINTS, NUM_IK_JOINTS, GRIPPER_INDEX,
...
class ForwardKinematics:
    ...
    def compute(self, joint_angles_deg: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        fk_state_angles[GRIPPER_INDEX] = 0.0  # gripper neutral so FK reflects arm pose, not jaw state
...
class IKSolver:
    ...
    def solve(self, target_position: np.ndarray, target_orientation_quat: Optional[np.ndarray], 
        ...
        ik_state_angles[GRIPPER_INDEX] = 0.0  # gripper neutral so IK state matches FK state
...
def vr_to_robot_coordinates(vr_pos: dict, scale: float = 1.0) -> np.ndarray:
    """
    Convert VR controller position delta to robot base-frame delta.

    WebXR frame:   +X = right, +Y = up, +Z = toward viewer (back)
    Robot frame:   +X = right, +Y = forward (away from base), +Z = up

    Mapping:
      robot +X  ←  VR +X   (same direction)
      robot +Y  ←  VR -Z   (hand moving forward = VR z decreases = robot Y increases)
      robot +Z  ←  VR +Y   (same direction)
    """
    return np.array([
        vr_pos['x'] * scale,    # VR +X (right)   → Robot +X (right)
        -vr_pos['z'] * scale,   # VR -Z (forward)  → Robot +Y (forward)
