...
class PyBulletVisualizer:
    ...
    def _can_use_display(self) -> bool:
        """Check if an X11 display is reachable."""
        if not os.environ.get('DISPLAY'):
            return False
        # Verify the X server actually responds.
        # We deliberately skip glxinfo — it is often not installed and its
        # absence doesn't mean OpenGL is unavailable (PyBullet uses libGL
        # directly, not the glxinfo CLI tool).
        try:
            import subprocess
            ...
            # xdpyinfo not installed — fall back to trusting $DISPLAY
            return True
