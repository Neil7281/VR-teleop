    ...
    this.connectWebSocket = async () => {
      ...
      try {
        ...
        if (config?.network?.websocket_port) {
          websocketPort = config.network.websocket_port;
        }
      } catch (error) {
        console.warn('Falling back to default WebSocket port 8442:', error);
      }
...
function createVrInstructionsPanel() {
    ...
    panel.innerHTML = `
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
          ...
          <h2 style="margin:0;font-size:1.15em;font-weight:800;">RoboInn — VR Controller Guide</h2>
        </div>
        <div style="display: flex; gap: 15px; align-items: flex-start; flex-wrap: wrap;">
            ...
            <div style="flex: 1; min-width: 200px; display: flex; flex-direction: column; gap: 8px; font-size: 14px;">
                <div style="padding: 8px; background: rgba(255,255,255,0.1); border-radius: 6px;">
                    <strong style="color: #ee4d9a;">Grip Button:</strong> Hold to track movement and move the arm
                </div>
                <div style="padding: 8px; background: rgba(255,255,255,0.1); border-radius: 6px;">
                    <strong style="color: #9af58c;">Trigger:</strong> Hold to close gripper; release to open
                </div>
            </div>
        </div>
    `;
