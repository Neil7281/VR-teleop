...
function restartSystem() {
  ...
  .then(response => {
    if (response.ok) {
      ...
      alert('RoboInn is restarting… The page will reload automatically in a few seconds.');
      closeSettings();
  ...
  .finally(() => {
    ...
    restartButton.textContent = '🔄 Restart System';  // reset label
  });
}
...
// Check if running in VR/AR mode (use A-Frame state, not fullscreen API)
function isVRMode() {
  const scene = document.querySelector('a-scene');
  return scene && (scene.is('vr-mode') || scene.is('ar-mode'));
}
...
function updateUIForDevice() {
  ...
  if (isVRMode()) {
    // In VR: hide desktop UI and the start button
  ...
  if (navigator.xr) {
    navigator.xr.isSessionSupported('immersive-vr').then((supported) => {
      if (supported) {
        // VR-capable device (Quest, etc.) — hide desktop UI and show the Start button
        desktopInterface.style.display = 'none';
        ...
        // Not a VR device — show the normal desktop interface
    ...
    // No WebXR support — show desktop interface
    desktopInterface.style.display = 'block';
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (navigator.xr) {
    ...
    navigator.xr.addEventListener('sessionend', () => {
      // Re-show start button and update status when user exits VR
      const startBtn = document.getElementById('start-tracking-button');
      if (startBtn) {
        startBtn.textContent = 'Start Controller Tracking';
